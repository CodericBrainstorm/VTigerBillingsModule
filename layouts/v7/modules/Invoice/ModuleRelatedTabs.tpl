	...
	<div class='related-tabs row'>
		<ul class="nav nav-tabs">
			...
			{/for}
				...
				<li class="tab-item {if ("Consign"== trim($SELECTED_TAB_LABEL))}active{/if}"
                                    data-url="module=Invoice&relatedModule=Invoice&view=Consign&record={$RECORD->getId()}&tab_label={$RELATED_LINK->getLabel()}&app={$SELECTED_MENU_CATEGORY}"
                                    data-label-key="{$RELATED_LINK->getLabel()}"
					data-module="Invoice" data-relation-id="{$RELATED_LINK->getId()}" title="Consign" {if $RELATEDFIELDNAME}data-relatedfield ="{$RELATEDFIELDNAME}"{/if}>
					<a href="index.php?module=Invoice&relatedModule=Invoice&view=Consign&record={$RECORD->getId()}&tab_label=Consign&app={$SELECTED_MENU_CATEGORY}" class="textOverflowEllipsis" displaylabel="Invoice" recordsCount="" >
						
                                                        <span class="tab-icon"><i class="vicon-subscription" title="Billings"></i></span>
